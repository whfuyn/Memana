...
// static表示不导出符号
static void ReorderBlockAscending(void* space, Block* curr);
...
void Free(void* space, void* ptr)
{
    ...
    {
        ReorderBlockAscending(space, pMergedBlock);
        return;
    }
    ...
    if(head){
        ...
        *GetPtrToHeadPtr(space) = curr;
        ...
        ReorderBlockAscending(space, curr);
    }
    else
    {
        PREV(curr) = NULL;
